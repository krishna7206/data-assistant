...
def extract(engine):
    ...
    for schema in schemas:
        for table_name in insp.get_table_names(schema=schema):
            ...
            # Dynamic safe row counting based on database dialect
            with engine.connect() as conn:
                ...
                try:
                    result = conn.execute(text(f"SELECT COUNT(*) FROM {quoted_target}"))
                    row_count = result.scalar()
                except Exception:
                    row_count = 0  # Fallback if permission rules or locked views block scanning
